...
def demo_basic():
    ...
    result = vectorstore.similarity_search("LangChain是什么", k=2)

    for i in enumerate(result):
        print(f"  [{i[0]}] {i[1].page_content}")
...
def demo_metadata_filter():
    ...
    vectorstore1 = Chroma.from_documents(documents, embeddings, collection_metadata=CHROMA_COLLECTION_METADATA)

    print("=== demo_2: metadata 过滤 ===")

    # 搜索 + 过滤
    result1 = vectorstore1.similarity_search(
        "帮我找找编程教程",
        k=5,
        filter={"lang": "python"}
    )
    ...
    # 多条件过率 $and $or
    result2 = vectorstore1.similarity_search(
        "帮我找找编程教程",
        k=5,
        filter={"$and": [
            {"lang": "python"},
